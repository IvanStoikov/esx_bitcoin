	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'cloakroom',
		{
			...
			elements = {
				...
				{label = _U('stop_mission'), value = 'stop_mission'}
			}
		},
		function(data, menu)
			...
			if data.current.value == 'job_wear' then
				...
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin)
					if skin.sex == 0 or 1 then
					SetModelAsNoLongerNeeded(model) -- Използва текущия модел на плейъра
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			ESX.Game.SpawnVehicle(data.current.value, Config.Zones.VehicleSpawnPoint.Pos, 270.0, function(vehicle)
				...
				xPlayer.removeAccountMoney('black_money', '500') -- Цена $ 500 Мръсни пари
...
function IsJobTrucker()
	if PlayerData ~= nil then
		...
		if PlayerData.job.name ~= nil or PlayerData.job.name ~= 'police' then -- Ако играчите работят ~ = нула и Името на длъжността на играча = 'РАБОТА, КОЯТО ИСКАШ ДА ИМАТ', тогава
			isJobTrucker = true
		end
		return isJobTrucker
	end
...
function retourcamion_oui()
	...
	livraisonnombre = 0
	MissionRegion = 0
	TriggerServerEvent("esx_godipizzeriamammamiartyjob:pay", amount)
